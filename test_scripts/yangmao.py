...
def getHeader(method,request_path,body):
	...
	mac = hmac.new(bytes(Secret_Key, encoding='utf8'), bytes(message, encoding='utf-8'), digestmod='sha256').digest()
...
def buy():
	...
	body = '{"base":"ADA","quote":"USDT","buyType":0,"price":"'+str(newPrice)+'","count":"1"}'
...
def sell():
	...
	url = dns + path
	newPrice=getNewprice()
	body = '{"base":"ADA","quote":"USDT","buyType":1,"price":"'+str(newPrice)+'","count":"1"}'
